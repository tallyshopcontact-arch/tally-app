...
export async function getChannelSubCounts(
  ...
  channelIds: string[]
): Promise<Map<string, { title: string; subscriberCount: number }>> {
  const distinct = [...new Set(channelIds.filter(Boolean))];
  const result = new Map<string, { title: string; subscriberCount: number }>();
  ...
  const cachedIds = new Set((cached ?? []).map((c) => c.channel_id as string));
  ...
  const toUpsert: { channel_id: string; title: string; subscriber_count: number; updated_at: string }[] = [];
