...
import type { ChannelAnalysis, NicheCandidate } from "@/lib/reports/channelAnalyzer";
...
function ReportBuilder({ password }: { password: string }) {
  ...
  // Niche picker (curator model) — Priority 1 & 2 are assigned from
  // analysis.nicheCandidates; Priority 3/Hold auto-fills from
  // analysis.defaultHoldCandidate. Each title format field starts pre-filled
  // from the candidate's winner-data example but stays fully editable.
  ...
  const [holdCandidate, setHoldCandidate] = useState<NicheCandidate | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // if it gets cleared out entirely.
      ...
      // Step 10 — Priority 3/Hold auto-fills from the channel's own
      // highest-opportunity niche, still editable same as a picked candidate.
      const hold = data.analysis?.defaultHoldCandidate ?? null;
      setHoldCandidate(hold);
  ...
  // a candidate already in the OTHER slot moves it rather than duplicating
  // it, and clicking a slot's own current candidate again un-assigns it.
  const assignPriority = (candidate: NicheCandidate, slot: 1 | 2) => {
    const isCurrent = (c: NicheCandidate | null) => c?.score.laneId === candidate.score.laneId;
    if (slot === 1) {
      if (isCurrent(priority1)) { setPriority1(null); setTitleFormat1(""); return; }
      setPriority1(candidate);
      setTitleFormat1(candidate.titleFormatExample);
      if (isCurrent(priority2)) { setPriority2(null); setTitleFormat2(""); }
    } else {
      if (isCurrent(priority2)) { setPriority2(null); setTitleFormat2(""); return; }
      setPriority2(candidate);
      setTitleFormat2(candidate.titleFormatExample);
      if (isCurrent(priority1)) { setPriority1(null); setTitleFormat1(""); }
    }
  };

  const bothPrioritiesAssigned = !!priority1 && !!priority2;
  ...
  const handleGenerate = async () => {
    ...
    try {
      const selectedPlan = {
        ...
        ...(holdCandidate
          ? { priority3: { laneId: holdCandidate.score.laneId, titleFormat: titleFormat3 } }
          : {}),
      };
        ...
        {analysis && !analyzing && (
          <div className="border border-[#1a1a1a] bg-[#0d0d0d] p-6 mb-8">
            ...
            <div className="border-t border-[#1a1a1a] pt-6 mb-6">
              <p className="text-sm font-bold text-white mb-1">Select Your 2 Priority Niches</p>
              <p className="text-xs text-[#94a3b8] mb-4">
                Ranked by opportunity — click a candidate to assign it to Priority 1 or 2. Each title format pre-fills from real winner data and stays editable.
              </p>

              <div className="grid grid-cols-2 gap-3 mb-4">
                {([[1, priority1, titleFormat1, setTitleFormat1], [2, priority2, titleFormat2, setTitleFormat2]] as const).map(
                  ([slotNum, sel, fmt, setFmt]) => (
                    <div key={slotNum} className="border border-[#1a1a1a] bg-[#111] p-4">
                      <p className="text-[10px] text-[#94a3b8] uppercase tracking-widest mb-2">Priority {slotNum}</p>
                      {sel ? (
                        <>
              ...
              <div className="border border-[#1a1a1a] bg-[#111] p-4">
                <p className="text-[10px] text-[#94a3b8] uppercase tracking-widest mb-2">Priority 3 · Hold</p>
                {holdCandidate ? (
                  <>
                    <p className="text-sm font-bold text-white mb-2">
                      {holdCandidate.score.artistName}{" "}
                      <span className="text-[#475569] font-normal text-xs">(auto-filled — your highest-opportunity current niche)</span>
                    </p>
                    <label className="block text-[10px] text-[#475569] uppercase tracking-widest mb-1">Title Format</label>
                    <textarea
                      value={titleFormat3}
                      onChange={(e) => setTitleFormat3(e.target.value)}
                      rows={2}
                      className="w-full bg-[#0a0a0a] border border-[#1e1e1e] px-2.5 py-2 text-xs text-white focus:outline-none focus:border-[#3a3a3a] transition-colors resize-none"
                    />
                  </>
                ) : (
                  <p className="text-xs text-[#475569]">No current scored niche to hold.</p>
                )}
              </div>
            </div>

            {bothPrioritiesAssigned && (
              <>
                <div className="mb-5">
                  <label className="block text-xs text-[#94a3b8] uppercase tracking-widest mb-2">
                    Experiment {experimentAutoGenerated ? "(drafted by TALLY — edit as needed)" : "(written by you)"}
                  </label>
                  <textarea
                    value={experiment}
                  ...
                  <p className={`text-[10px] mt-1.5 ${experiment.trim() ? "text-[#475569]" : "text-[#f87171]"}`}>
                    {experiment.trim() ? "This becomes the report's Experiment section." : "Add an experiment before generating"}
                  </p>
                </div>
